(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))r(n);new MutationObserver(n=>{for(const l of n)if(l.type==="childList")for(const i of l.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&r(i)}).observe(document,{childList:!0,subtree:!0});function a(n){const l={};return n.integrity&&(l.integrity=n.integrity),n.referrerPolicy&&(l.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?l.credentials="include":n.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function r(n){if(n.ep)return;n.ep=!0;const l=a(n);fetch(n.href,l)}})();const T={};class B{constructor(){this.supportedLanguages=["en","es","de","fr","it","pt","zh"],this.currentLanguage=this.detectLanguage(),this.cache={},this.loadedNamespaces=new Set(["common"]),this.listeners=[]}detectLanguage(){const e=localStorage.getItem("passq_lang");if(e&&this.supportedLanguages.includes(e))return e;const a=(navigator.language||"en").split("-")[0].toLowerCase();return this.supportedLanguages.includes(a)?a:"en"}async setLanguage(e){if(this.supportedLanguages.includes(e)){this.currentLanguage=e,localStorage.setItem("passq_lang",e),document.documentElement.lang=e,this.loadedNamespaces.size>0&&await this.loadNamespaces(Array.from(this.loadedNamespaces));for(const a of this.listeners)try{await a(e)}catch(r){console.warn("Error in language change listener:",r)}}}onLanguageChange(e){this.listeners.push(e)}async loadNamespaces(e=["common"]){const a=this.currentLanguage;this.cache[a]||(this.cache[a]={}),this.cache.en||(this.cache.en={});const r="./",n=r.endsWith("/")?r:r+"/";for(const l of e){if(this.loadedNamespaces.add(l),!this.cache[a][l])try{const i=await fetch(`${n}locales/${a}/${l}.json?v=20260913_5`);i.ok&&(this.cache[a][l]=await i.json())}catch{console.warn(`Could not load locales/${a}/${l}.json, falling back to English`)}if(a!=="en"&&!this.cache.en[l])try{const i=await fetch(`${n}locales/en/${l}.json?v=20260913_5`);i.ok&&(this.cache.en[l]=await i.json())}catch{console.error(`Could not load fallback locales/en/${l}.json`)}}}t(e,a=""){if(!e)return a;const[r,n]=e.includes(":")?e.split(":"):["common",e],l=this.currentLanguage;let i=this._getValue(this.cache[l]?.[r],n);return i!==void 0||(i=this._getValue(this.cache.en?.[r],n),i!==void 0)?i:a||n}_getValue(e,a){if(e)return a.split(".").reduce((r,n)=>r&&r[n],e)}translateDOM(){document.querySelectorAll("[data-i18n]").forEach(e=>{const a=e.getAttribute("data-i18n");e.hasAttribute("data-i18n-original")||e.setAttribute("data-i18n-original",e.textContent.trim());const r=e.getAttribute("data-i18n-original")||"",n=this.t(a,r);e.tagName==="INPUT"||e.tagName==="TEXTAREA"?e.placeholder=n:e.textContent=n}),document.querySelectorAll("[data-i18n-placeholder]").forEach(e=>{e.placeholder=this.t(e.getAttribute("data-i18n-placeholder"),e.placeholder)}),document.querySelectorAll("[data-i18n-title]").forEach(e=>{e.title=this.t(e.getAttribute("data-i18n-title"),e.title)}),document.querySelectorAll("[data-i18n-aria-label]").forEach(e=>{e.setAttribute("aria-label",this.t(e.getAttribute("data-i18n-aria-label"),e.getAttribute("aria-label")||""))}),document.querySelectorAll("[data-i18n-html]").forEach(e=>{e.hasAttribute("data-i18n-original-html")||e.setAttribute("data-i18n-original-html",e.innerHTML.trim());const a=e.getAttribute("data-i18n-original-html")||"";e.innerHTML=this.t(e.getAttribute("data-i18n-html"),a)})}}const s=new B,o={sun:(t="w-5 h-5")=>`
      ...
      <div class="relative w-full max-w-3xl max-h-[88vh] max-h-[88dvh] overflow-hidden rounded-3xl bg-white/95 dark:bg-[#0c1322]/95 border border-slate-200 dark:border-white/15 shadow-2xl backdrop-blur-2xl z-10 animate-in fade-in zoom-in-95 duration-200 flex flex-col">
        ...
        <div class="px-5 sm:px-6 py-3 border-b border-slate-200/60 dark:border-white/10 bg-slate-50/70 dark:bg-white/[0.02] overflow-x-auto no-scrollbar shrink-0">
          <div class="flex items-center gap-2.5 min-w-max">
            ${i.map(c=>{const m=this.activeTab===c.id;return`
                <button type="button" data-tab-target="${c.id}" class="guide-tab-btn flex items-center gap-2 px-4 py-2.5 rounded-2xl text-sm sm:text-[15px] font-bold transition cursor-pointer select-none ${m?"bg-emerald-600 text-white shadow-md shadow-emerald-600/20":"bg-white/80 dark:bg-white/5 text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-white/10 border border-slate-200/80 dark:border-white/10"}">
                  <span class="icon-svg w-4 h-4 shrink-0 ${m?"text-white":"text-emerald-500 dark:text-emerald-400"}">
    ...
    `,e.querySelector("#user-guide-backdrop")?.addEventListener("click",()=>this.close()),e.querySelector("#btn-close-guide-top")?.addEventListener("click",()=>this.close()),e.querySelector("#btn-guide-close-bottom")?.addEventListener("click",()=>this.close()),e.querySelectorAll(".guide-tab-btn").forEach(c=>{c.addEventListener("click",()=>{const m=c.getAttribute("data-tab-target");m&&this.setTab(m)})});const d=i.findIndex(c=>c.id===this.activeTab),h=e.querySelector("#btn-guide-prev"),p=e.querySelector("#btn-guide-next");h&&(h.disabled=d<=0,h.addEventListener("click",()=>{d>0&&this.setTab(i[d-1].id)})),p&&(p.disabled=d>=i.length-1,p.addEventListener("click",()=>{d<i.length-1&&this.setTab(i[d+1].id)}))}renderTabContent(){switch(this.activeTab){case"tab1":return`
          <div class="space-y-4">
      ...
      <div class="relative w-full max-w-2xl max-h-[88vh] max-h-[88dvh] overflow-y-auto overscroll-contain rounded-3xl bg-white/95 dark:bg-[#0c1322]/95 border border-slate-200 dark:border-white/15 shadow-2xl p-5 sm:p-8 backdrop-blur-2xl z-10 animate-in fade-in zoom-in-95 duration-200 flex flex-col gap-5">
        ...
        <div class="p-4 rounded-2xl bg-gradient-to-r from-emerald-500/15 to-teal-500/15 border border-emerald-500/30 flex items-center gap-3.5">
          ...
          <div class="text-xs sm:text-sm">
            ...
            <div class="text-slate-600 dark:text-slate-300 mt-0.5 font-medium">${m}</div>
          </div>
        </div>
    ...
    `,e.querySelector("#subscription-modal-backdrop")?.addEventListener("click",()=>this.close()),e.querySelector("#btn-close-sub-modal")?.addEventListener("click",()=>this.close()),e.querySelectorAll("[data-plan-select]").forEach(u=>{u.addEventListener("click",()=>{this.selectedPlan=u.getAttribute("data-plan-select"),this.render()})}),e.querySelector("#btn-confirm-paypal-subscribe")?.addEventListener("click",()=>{this.billing.launchPayPalPurchase(this.selectedPlan),this.close()}),e.querySelector("#btn-confirm-play-subscribe")?.addEventListener("click",async()=>{await this.billing.launchGooglePlayPurchase(this.selectedPlan),this.close()})}}const O=new y,N=new Z,x=[{code:"es",short:"ES",name:"Español"},{code:"en",short:"EN",name:"English"},{code:"de",short:"DE",name:"Deutsch"},{code:"fr",short:"FR",name:"Français"},{code:"it",short:"IT",name:"Italiano"},{code:"pt",short:"PT",name:"Português"},{code:"zh",short:"ZH",name:"中文"}];class D{constructor(e="#lang-select"){this.target=typeof e=="string"?document.querySelector(e):e,this.container=null,this.isOpen=!1,this.init()}init(){const e=document.getElementById("custom-lang-dropdown-wrapper");if(e){this.container=e,this.updateActiveState(s.currentLanguage);return}if(!this.target)return;const a=this.target.parentElement;!a||!a.parentElement||(this.container=document.createElement("div"),this.container.className="relative inline-block text-left z-50 shrink-0",this.container.id="custom-lang-dropdown-wrapper",a.parentElement.replaceChild(this.container,a),this.render(),this.setupListeners(),s.onLanguageChange(r=>{this.updateActiveState(r)}))}render(){const e=s.currentLanguage||"es",a=x.find(n=>n.code===e)||x[0],r=o[`flag_${a.code}`]?o[`flag_${a.code}`]("w-4 h-3 rounded-xs shadow-xs shrink-0"):"";this.container.innerHTML=`
      ...
      <button id="custom-lang-btn" type="button" class="h-10 sm:h-11 px-2.5 sm:px-3.5 rounded-2xl bg-white/85 dark:bg-slate-900/85 border border-slate-200 dark:border-white/10 shadow-sm backdrop-blur-md hover:bg-white dark:hover:bg-slate-800 active:scale-95 text-slate-800 dark:text-slate-100 flex items-center gap-1.5 sm:gap-2 transition cursor-pointer select-none shrink-0 whitespace-nowrap" aria-haspopup="true" aria-expanded="false">
        <span id="custom-lang-current-flag" class="flex items-center shrink-0">${r}</span>
      ...
      <div id="custom-lang-menu" class="hidden opacity-0 scale-95 transition-all duration-150 ease-out absolute right-0 top-full mt-2 w-48 sm:w-52 p-1.5 rounded-2xl bg-white/95 dark:bg-[#0c1322]/95 border border-slate-200/90 dark:border-white/15 shadow-2xl backdrop-blur-2xl z-[9999] origin-top-right">
        <div class="space-y-1" role="menu" aria-orientation="vertical">
          ${x.map(n=>{const l=n.code===e,i=o[`flag_${n.code}`]?o[`flag_${n.code}`]("w-4 h-3 rounded-xs shadow-xs shrink-0"):"";return`
    ...
    `}setupListeners(){const e=this.container.querySelector("#custom-lang-btn"),a=this.container.querySelector("#custom-lang-menu");!e||!a||(e.addEventListener("click",r=>{r.stopPropagation(),this.toggleMenu()}),a.addEventListener("click",r=>{const n=r.target.closest(".lang-option-btn");if(!n)return;r.stopPropagation();const l=n.getAttribute("data-lang-code");l&&(s.setLanguage(l),this.closeMenu())}),document.addEventListener("click",r=>{this.isOpen&&!this.container.contains(r.target)&&this.closeMenu()}),document.addEventListener("keydown",r=>{r.key==="Escape"&&this.isOpen&&this.closeMenu()}))}toggleMenu(){this.isOpen?this.closeMenu():this.openMenu()}openMenu(){const e=this.container?.querySelector("#custom-lang-menu"),a=this.container?.querySelector("#custom-lang-chevron"),r=this.container?.querySelector("#custom-lang-btn");if(!e)return;this.isOpen=!0,this.container&&(this.container.style.zIndex="9999");const n=this.container.closest("header")||this.container.closest(".glass-card");n&&(n.style.zIndex="999"),e.classList.remove("hidden"),r&&r.setAttribute("aria-expanded","true"),requestAnimationFrame(()=>{const l=e.getBoundingClientRect();l.left<8?(e.classList.remove("right-0","origin-top-right"),e.classList.add("left-0","origin-top-left")):l.right>window.innerWidth-8&&(e.classList.remove("left-0","origin-top-left"),e.classList.add("right-0","origin-top-right")),e.classList.remove("opacity-0","scale-95"),e.classList.add("opacity-100","scale-100"),a&&a.classList.add("rotate-180")})}closeMenu(){const e=this.container?.querySelector("#custom-lang-menu"),a=this.container?.querySelector("#custom-lang-chevron"),r=this.container?.querySelector("#custom-lang-btn");e&&(this.isOpen=!1,r&&r.setAttribute("aria-expanded","false"),e.classList.remove("opacity-100","scale-100"),e.classList.add("opacity-0","scale-95"),a&&a.classList.remove("rotate-180"),setTimeout(()=>{if(!this.isOpen){e.classList.add("hidden"),this.container&&(this.container.style.zIndex="");const n=this.container?.closest("header")||this.container?.closest(".glass-card");n&&(n.style.zIndex="")}},150))}updateActiveState(e){const a=x.find(i=>i.code===e)||x[0],r=this.container?.querySelector("#custom-lang-current-flag"),n=this.container?.querySelector("#custom-lang-current-label");r&&o[`flag_${a.code}`]&&(r.innerHTML=o[`flag_${a.code}`]("w-4 h-3 rounded-xs shadow-xs shrink-0")),n&&(n.textContent=a.short);const l=this.container?.querySelector("#custom-lang-menu > div");l&&(l.innerHTML=x.map(i=>{const d=i.code===e,h=o[`flag_${i.code}`]?o[`flag_${i.code}`]("w-4 h-3 rounded-xs shadow-xs shrink-0"):"";return`
