(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))o(r);new MutationObserver(r=>{for(const i of r)if(i.type==="childList")for(const l of i.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&o(l)}).observe(document,{childList:!0,subtree:!0});function a(r){const i={};return r.integrity&&(i.integrity=r.integrity),r.referrerPolicy&&(i.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?i.credentials="include":r.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function o(r){if(r.ep)return;r.ep=!0;const i=a(r);fetch(r.href,i)}})();const E={};class A{constructor(){this.supportedLanguages=["en","es","de","fr","it","pt","zh"],this.currentLanguage=this.detectLanguage(),this.cache={},this.loadedNamespaces=new Set(["common"]),this.listeners=[]}detectLanguage(){const e=localStorage.getItem("passq_lang");if(e&&this.supportedLanguages.includes(e))return e;const a=(navigator.language||"en").split("-")[0].toLowerCase();return this.supportedLanguages.includes(a)?a:"en"}async setLanguage(e){if(this.supportedLanguages.includes(e)){this.currentLanguage=e,localStorage.setItem("passq_lang",e),document.documentElement.lang=e,this.loadedNamespaces.size>0&&await this.loadNamespaces(Array.from(this.loadedNamespaces));for(const a of this.listeners)try{await a(e)}catch(o){console.warn("Error in language change listener:",o)}}}onLanguageChange(e){this.listeners.push(e)}async loadNamespaces(e=["common"]){const a=this.currentLanguage;this.cache[a]||(this.cache[a]={}),this.cache.en||(this.cache.en={});const o="./",r=o.endsWith("/")?o:o+"/";for(const i of e){if(this.loadedNamespaces.add(i),!this.cache[a][i])try{const l=await fetch(`${r}locales/${a}/${i}.json`);l.ok&&(this.cache[a][i]=await l.json())}catch{console.warn(`Could not load locales/${a}/${i}.json, falling back to English`)}if(a!=="en"&&!this.cache.en[i])try{const l=await fetch(`${r}locales/en/${i}.json`);l.ok&&(this.cache.en[i]=await l.json())}catch{console.error(`Could not load fallback locales/en/${i}.json`)}}}t(e,a=""){if(!e)return a;const[o,r]=e.includes(":")?e.split(":"):["common",e],i=this.currentLanguage;let l=this._getValue(this.cache[i]?.[o],r);return l!==void 0||(l=this._getValue(this.cache.en?.[o],r),l!==void 0)?l:a||r}_getValue(e,a){if(e)return a.split(".").reduce((o,r)=>o&&o[r],e)}translateDOM(){document.querySelectorAll("[data-i18n]").forEach(e=>{const a=e.getAttribute("data-i18n"),o=this.t(a);e.tagName==="INPUT"||e.tagName==="TEXTAREA"?e.placeholder=o:e.textContent=o}),document.querySelectorAll("[data-i18n-placeholder]").forEach(e=>{e.placeholder=this.t(e.getAttribute("data-i18n-placeholder"))}),document.querySelectorAll("[data-i18n-title]").forEach(e=>{e.title=this.t(e.getAttribute("data-i18n-title"))}),document.querySelectorAll("[data-i18n-aria-label]").forEach(e=>{e.setAttribute("aria-label",this.t(e.getAttribute("data-i18n-aria-label")))}),document.querySelectorAll("[data-i18n-html]").forEach(e=>{e.innerHTML=this.t(e.getAttribute("data-i18n-html"))})}}const s=new A,n={sun:(t="w-5 h-5")=>`
      ...
      <div class="relative w-full max-w-2xl max-h-[92vh] overflow-y-auto rounded-3xl bg-white/95 dark:bg-[#0c1322]/95 border border-slate-200 dark:border-white/15 shadow-2xl p-6 sm:p-8 backdrop-blur-2xl z-10 animate-in fade-in zoom-in-95 duration-200 flex flex-col gap-5">
        ...
        <div class="flex items-center justify-between">
          <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 text-xs font-black tracking-wide">
            <span class="w-4 h-4 flex items-center">${n.googlePlay?n.googlePlay("w-4 h-4"):"▶"}</span>
            <span>${o}</span>
          </div>

          <button id="btn-close-sub-modal" type="button" class="w-9 h-9 rounded-xl bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 text-slate-500 dark:text-slate-300 flex items-center justify-center transition cursor-pointer" aria-label="${k}">
            ${n.close?n.close("w-5 h-5"):"✕"}
          </button>
        </div>
        ...
        <div class="p-4 rounded-2xl bg-gradient-to-r from-emerald-500/15 to-teal-500/15 border border-emerald-500/30 flex items-center gap-3.5">
          ...
          <div class="text-xs sm:text-sm">
            <div class="font-extrabold text-slate-900 dark:text-white">${u}</div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
          ${H.map(h=>{const L=this.selectedPlan===h.id,S=s.t(`common:billing.plans.${h.nameKey}.name`,h.nameKey.toUpperCase()),j=s.t(`common:billing.plans.${h.nameKey}.limit`,"PassQ Pro");return`
              <div data-plan-select="${h.id}" class="p-4 rounded-2xl border-2 transition-all cursor-pointer flex flex-col justify-between relative select-none ${L?"border-emerald-500 bg-emerald-500/10 shadow-lg shadow-emerald-500/10":"border-slate-200/80 dark:border-white/10 bg-slate-50 dark:bg-white/5 hover:border-slate-300 dark:hover:border-white/20"}">
                ${h.recommended?`
                ...
                <div class="space-y-1.5">
                  <div class="text-xs font-black uppercase text-slate-500 dark:text-slate-400">${S}</div>
                  <div class="text-lg sm:text-xl font-black text-slate-900 dark:text-white">${h.price}</div>
                  <div class="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 font-mono">${j}</div>
                </div>
        ...
        <div class="p-3.5 rounded-2xl bg-slate-100/80 dark:bg-white/5 border border-slate-200/80 dark:border-white/10 text-xs text-slate-600 dark:text-slate-300 space-y-1.5">
          ...
          <ul class="list-disc pl-5 space-y-1 leading-relaxed text-[11.5px]">
            <li>${g}</li>
            <li>${b}<a href="https://play.google.com/store/account/subscriptions" target="_blank" class="text-emerald-500 underline font-bold">${$}</a>${_}</li>
            <li>${M}</li>
          </ul>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-2">
          <button id="btn-confirm-play-subscribe" type="button" class="flex-1 py-3.5 px-6 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-sm sm:text-base shadow-xl shadow-emerald-600/30 transition active:scale-[0.98] flex items-center justify-center gap-2.5 cursor-pointer">
            <span class="w-5 h-5 flex items-center">${n.googlePlay?n.googlePlay("w-5 h-5"):"▶"}</span>
            <span>${C}</span>
          </button>
          ...
          ${x.map(r=>{const i=r.code===e,l=n[`flag_${r.code}`]?n[`flag_${r.code}`]("w-4 h-3 rounded-xs shadow-xs shrink-0"):"";return`
              <button type="button" data-lang-code="${r.code}" class="lang-option-btn w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs sm:text-sm font-medium transition cursor-pointer select-none ${i?"bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 font-bold border border-emerald-500/20":"text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-white/5 border border-transparent"}">
                <div class="flex items-center gap-2.5">
                  <span class="flex items-center shrink-0">${l}</span>
                  <span>${r.name}</span>
                </div>
    ...
    `}setupListeners(){const e=this.container.querySelector("#custom-lang-btn"),a=this.container.querySelector("#custom-lang-menu");!e||!a||(e.addEventListener("click",o=>{o.stopPropagation(),this.toggleMenu()}),a.addEventListener("click",o=>{const r=o.target.closest(".lang-option-btn");if(!r)return;o.stopPropagation();const i=r.getAttribute("data-lang-code");i&&(s.setLanguage(i),this.closeMenu())}),document.addEventListener("click",o=>{this.isOpen&&!this.container.contains(o.target)&&this.closeMenu()}),document.addEventListener("keydown",o=>{o.key==="Escape"&&this.isOpen&&this.closeMenu()}))}toggleMenu(){this.isOpen?this.closeMenu():this.openMenu()}openMenu(){const e=this.container?.querySelector("#custom-lang-menu"),a=this.container?.querySelector("#custom-lang-chevron"),o=this.container?.querySelector("#custom-lang-btn");e&&(this.isOpen=!0,e.classList.remove("hidden"),o&&o.setAttribute("aria-expanded","true"),requestAnimationFrame(()=>{e.classList.remove("opacity-0","scale-95"),e.classList.add("opacity-100","scale-100"),a&&a.classList.add("rotate-180")}))}closeMenu(){const e=this.container?.querySelector("#custom-lang-menu"),a=this.container?.querySelector("#custom-lang-chevron"),o=this.container?.querySelector("#custom-lang-btn");e&&(this.isOpen=!1,o&&o.setAttribute("aria-expanded","false"),e.classList.remove("opacity-100","scale-100"),e.classList.add("opacity-0","scale-95"),a&&a.classList.remove("rotate-180"),setTimeout(()=>{this.isOpen||e.classList.add("hidden")},150))}updateActiveState(e){const a=x.find(l=>l.code===e)||x[0],o=this.container?.querySelector("#custom-lang-current-flag"),r=this.container?.querySelector("#custom-lang-current-label");o&&n[`flag_${a.code}`]&&(o.innerHTML=n[`flag_${a.code}`]("w-4 h-3 rounded-xs shadow-xs shrink-0")),r&&(r.textContent=a.short);const i=this.container?.querySelector("#custom-lang-menu > div");i&&(i.innerHTML=x.map(l=>{const d=l.code===e,m=n[`flag_${l.code}`]?n[`flag_${l.code}`]("w-4 h-3 rounded-xs shadow-xs shrink-0"):"";return`
          <button type="button" data-lang-code="${l.code}" class="lang-option-btn w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs sm:text-sm font-medium transition cursor-pointer select-none ${d?"bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 font-bold border border-emerald-500/20":"text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-white/5 border border-transparent"}">
            ...
            <div class="flex items-center gap-1.5">
              <span class="text-[10px] font-mono uppercase font-bold text-slate-400 dark:text-slate-500">${l.short}</span>
              ${d?`
                <span class="icon-svg w-3.5 h-3.5 text-emerald-500">
                  <svg fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5"/></svg>
                </span>
...
`)),f=`mailto:${d}?subject=${p}&body=${v}`;e.className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-950/70 backdrop-blur-md transition-opacity duration-200",e.innerHTML=`
      ...
      <div class="relative w-full max-w-xl overflow-hidden rounded-3xl bg-white/95 dark:bg-[#0c1322]/95 border border-slate-200 dark:border-white/15 shadow-2xl p-6 sm:p-8 backdrop-blur-2xl z-10 animate-in fade-in zoom-in-95 duration-200 flex flex-col gap-5">
        ...
        <div class="p-4 sm:p-5 rounded-2xl bg-emerald-500/5 dark:bg-emerald-500/10 border border-emerald-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div class="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">${l}</div>
