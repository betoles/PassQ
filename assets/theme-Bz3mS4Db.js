(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const l of n)if(l.type==="childList")for(const i of l.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function a(n){const l={};return n.integrity&&(l.integrity=n.integrity),n.referrerPolicy&&(l.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?l.credentials="include":n.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function o(n){if(n.ep)return;n.ep=!0;const l=a(n);fetch(n.href,l)}})();const T={};class B{constructor(){this.supportedLanguages=["en","es","de","fr","it","pt","zh"],this.currentLanguage=this.detectLanguage(),this.cache={},this.loadedNamespaces=new Set(["common"]),this.listeners=[]}detectLanguage(){const e=localStorage.getItem("passq_lang");if(e&&this.supportedLanguages.includes(e))return e;const a=(navigator.language||"en").split("-")[0].toLowerCase();return this.supportedLanguages.includes(a)?a:"en"}async setLanguage(e){if(this.supportedLanguages.includes(e)){this.currentLanguage=e,localStorage.setItem("passq_lang",e),document.documentElement.lang=e,this.loadedNamespaces.size>0&&await this.loadNamespaces(Array.from(this.loadedNamespaces));for(const a of this.listeners)try{await a(e)}catch(o){console.warn("Error in language change listener:",o)}}}onLanguageChange(e){this.listeners.push(e)}async loadNamespaces(e=["common"]){const a=this.currentLanguage;this.cache[a]||(this.cache[a]={}),this.cache.en||(this.cache.en={});const o="./",n=o.endsWith("/")?o:o+"/";for(const l of e){if(this.loadedNamespaces.add(l),!this.cache[a][l])try{const i=await fetch(`${n}locales/${a}/${l}.json?v=20260913_5`);i.ok&&(this.cache[a][l]=await i.json())}catch{console.warn(`Could not load locales/${a}/${l}.json, falling back to English`)}if(a!=="en"&&!this.cache.en[l])try{const i=await fetch(`${n}locales/en/${l}.json?v=20260913_5`);i.ok&&(this.cache.en[l]=await i.json())}catch{console.error(`Could not load fallback locales/en/${l}.json`)}}}t(e,a=""){if(!e)return a;const[o,n]=e.includes(":")?e.split(":"):["common",e],l=this.currentLanguage;let i=this._getValue(this.cache[l]?.[o],n);return i!==void 0||(i=this._getValue(this.cache.en?.[o],n),i!==void 0)?i:a||n}_getValue(e,a){if(e)return a.split(".").reduce((o,n)=>o&&o[n],e)}translateDOM(){document.querySelectorAll("[data-i18n]").forEach(e=>{const a=e.getAttribute("data-i18n");e.hasAttribute("data-i18n-original")||e.setAttribute("data-i18n-original",e.textContent.trim());const o=e.getAttribute("data-i18n-original")||"",n=this.t(a,o);e.tagName==="INPUT"||e.tagName==="TEXTAREA"?e.placeholder=n:e.textContent=n}),document.querySelectorAll("[data-i18n-placeholder]").forEach(e=>{e.placeholder=this.t(e.getAttribute("data-i18n-placeholder"),e.placeholder)}),document.querySelectorAll("[data-i18n-title]").forEach(e=>{e.title=this.t(e.getAttribute("data-i18n-title"),e.title)}),document.querySelectorAll("[data-i18n-aria-label]").forEach(e=>{e.setAttribute("aria-label",this.t(e.getAttribute("data-i18n-aria-label"),e.getAttribute("aria-label")||""))}),document.querySelectorAll("[data-i18n-html]").forEach(e=>{e.hasAttribute("data-i18n-original-html")||e.setAttribute("data-i18n-original-html",e.innerHTML.trim());const a=e.getAttribute("data-i18n-original-html")||"";e.innerHTML=this.t(e.getAttribute("data-i18n-html"),a)})}}const s=new B,r={sun:(t="w-5 h-5")=>`
        ...
        <div class="p-5 sm:p-6 border-b border-slate-200/80 dark:border-white/10 flex items-start justify-between gap-4 shrink-0">
          <div class="space-y-1.5">
            <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs sm:text-sm font-bold tracking-wide">
              <span class="w-4 h-4 flex items-center">${r.bookOpen?r.bookOpen("w-4 h-4"):""}</span>
              <span>${o}</span>
            </div>
            <h2 class="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2">
              <span>${a}</span>
            </h2>
            <p class="text-sm sm:text-base font-medium text-slate-600 dark:text-slate-300 leading-relaxed">${n}</p>
          </div>
    ...
    `,e.querySelector("#user-guide-backdrop")?.addEventListener("click",()=>this.close()),e.querySelector("#btn-close-guide-top")?.addEventListener("click",()=>this.close()),e.querySelector("#btn-guide-close-bottom")?.addEventListener("click",()=>this.close()),e.querySelectorAll(".guide-tab-btn").forEach(c=>{c.addEventListener("click",()=>{const h=c.getAttribute("data-tab-target");h&&this.setTab(h)})});const d=i.findIndex(c=>c.id===this.activeTab),m=e.querySelector("#btn-guide-prev"),u=e.querySelector("#btn-guide-next");m&&(m.disabled=d<=0,m.addEventListener("click",()=>{d>0&&this.setTab(i[d-1].id)})),u&&(u.disabled=d>=i.length-1,u.addEventListener("click",()=>{d<i.length-1&&this.setTab(i[d+1].id)}))}renderTabContent(){switch(this.activeTab){case"tab1":return`
          <div class="space-y-4">
            <div class="p-4 sm:p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-950 dark:text-emerald-200">
              <h3 class="text-lg sm:text-xl font-black mb-2 flex items-start sm:items-center gap-2.5 leading-snug">
                <span class="w-6 h-6 flex items-center text-emerald-500 shrink-0 mt-0.5 sm:mt-0">${r.sparkles?r.sparkles("w-6 h-6"):"★"}</span>
          ...
          <div class="space-y-4">
            <div class="p-4 sm:p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-950 dark:text-emerald-200">
              <h3 class="text-lg sm:text-xl font-black flex items-start sm:items-center gap-2.5 leading-snug">
                <span class="w-6 h-6 flex items-center text-emerald-500 shrink-0 mt-0.5 sm:mt-0">${r.plusCircle?r.plusCircle("w-6 h-6"):""}</span>
        ...
        `;case"tab3":return`
          <div class="space-y-4">
            <h3 class="text-lg sm:text-xl font-black text-slate-900 dark:text-white flex items-start sm:items-center gap-2.5 leading-snug">
              <span class="w-6 h-6 flex items-center text-emerald-500 shrink-0 mt-0.5 sm:mt-0">${r.folder?r.folder("w-6 h-6"):""}</span>
        ...
        `;case"tab4":return`
          <div class="space-y-4">
            <h3 class="text-lg sm:text-xl font-black text-slate-900 dark:text-white flex items-start sm:items-center gap-2.5 leading-snug">
              <span class="w-6 h-6 flex items-center text-emerald-500 shrink-0 mt-0.5 sm:mt-0">${r.devicePhoneMobile?r.devicePhoneMobile("w-6 h-6"):""}</span>
            ...
            <div class="p-4 sm:p-5 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 space-y-2 text-cyan-950 dark:text-cyan-200">
              <h4 class="font-black text-base sm:text-lg uppercase tracking-wide flex items-center gap-2.5">
                <span class="w-5 h-5 text-cyan-500">${r.userSlash?r.userSlash("w-5 h-5"):""}</span>
            ...
            <div class="p-4 sm:p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 space-y-2 text-emerald-950 dark:text-emerald-200">
              <h4 class="font-black text-base sm:text-lg uppercase tracking-wide flex items-center gap-2.5">
                <span class="w-5 h-5 text-emerald-500">${r.shieldCheck?r.shieldCheck("w-5 h-5"):""}</span>
        ...
        `;case"tab5":return`
          <div class="space-y-4">
            <h3 class="text-lg sm:text-xl font-black text-slate-900 dark:text-white flex items-start sm:items-center gap-2.5 leading-snug">
              <span class="w-6 h-6 flex items-center text-emerald-500 shrink-0 mt-0.5 sm:mt-0">${r.shieldCheck?r.shieldCheck("w-6 h-6"):""}</span>
        ...
        `;default:return""}}}new H;const q="EQNTHLAVHUL52",k=[{id:"passq_starter_monthly",nameKey:"starter",name:"Starter Plan",price:"$19.99 USD",amount:"19.99",period:"/ mes",trialDays:7},{id:"passq_growth_monthly",nameKey:"growth",name:"Growth Plan",price:"$39.99 USD",amount:"39.99",period:"/ mes",recommended:!0,trialDays:7},{id:"passq_scale_monthly",nameKey:"scale",name:"Scale Plan",price:"$99.99 USD",amount:"99.99",period:"/ mes",trialDays:7}];class y{constructor(){}startTrial(){typeof window>"u"||localStorage.getItem("passq_trial_start")||localStorage.setItem("passq_trial_start",Date.now().toString())}getTrialStatus(){if(typeof window>"u")return{day:1,daysRemaining:7,isTrialActive:!0,isTrialStarted:!1,isSubscribed:!1};const e=localStorage.getItem("passq_play_subscription_active")==="true",a=localStorage.getItem("passq_play_subscription_plan")||"growth",o=localStorage.getItem("passq_trial_start");if(!o)return{day:0,daysRemaining:7,isTrialActive:!0,isTrialStarted:!1,isSubscribed:e,activePlan:a,isAccessGranted:!0};const n=Date.now()-parseInt(o,10),l=Math.min(7,Math.max(1,Math.floor(n/(1e3*60*60*24))+1)),i=Math.max(0,7-Math.floor(n/(1e3*60*60*24))),d=i>0;return{day:l,daysRemaining:i,isTrialActive:d,isTrialStarted:!0,isSubscribed:e,activePlan:a,isAccessGranted:d||e}}activateSubscription(e="passq_growth_monthly"){const a=this.getTrialStatus(),o=Date.now();let n;a.isTrialStarted?n=(parseInt(localStorage.getItem("passq_trial_start"),10)||o)+7*24*60*60*1e3:(n=o+7*24*60*60*1e3,localStorage.setItem("passq_trial_start",o.toString()));const l=Math.max(o,n),i=l+30*24*60*60*1e3;localStorage.setItem("passq_play_subscription_active","true"),localStorage.setItem("passq_play_subscription_plan",e),localStorage.setItem("passq_subscription_first_charge_date",l.toString()),localStorage.setItem("passq_subscription_renewal_date",i.toString())}async launchGooglePlayPurchase(e="passq_growth_monthly"){if("getDigitalGoodsService"in window)try{const o=await window.getDigitalGoodsService("https://play.google.com/billing");if(o){const n=await o.getDetails([e]);if(n&&n.length>0){const l=[{supportedMethods:"https://play.google.com/billing",data:{sku:e}}];return await(await new PaymentRequest(l).show()).complete("success"),this.activateSubscription(e),{success:!0,method:"digital_goods_api"}}}}catch(o){console.warn("Google Play Digital Goods API flow:",o)}return window.open("https://play.google.com/store/account/subscriptions","_blank","noopener,noreferrer"),{success:!0,method:"play_store_external"}}launchPayPalPurchase(e="passq_growth_monthly"){const a=k.find(h=>h.id===e)||k[1],n=(typeof window<"u"?window.location.href:"https://betoles.github.io/PassQ/").split("?")[0].split("#")[0],l=n.replace(/\/[^/]*$/,"/app.html?billing=paypal_success"),i=n.replace(/\/[^/]*$/,"/app.html?billing=paypal_cancel"),d=this.getTrialStatus(),m=d.isTrialStarted?Math.max(1,d.daysRemaining):7,c=`https://www.paypal.com/cgi-bin/webscr?${new URLSearchParams({cmd:"_xclick-subscriptions",business:q,item_name:`PassQ DPP - ${a.name} ($${a.amount} USD/mo)`,item_number:a.id,no_shipping:"1",no_note:"1",currency_code:"USD",a1:"0",p1:m.toString(),t1:"D",a3:a.amount,p3:"1",t3:"M",src:"1",sra:"1",return:l,cancel_return:i}).toString()}`;return window.open(c,"_blank","noopener,noreferrer"),{success:!0,method:"paypal_web",url:c}}}class Z{constructor(){this.modalEl=null,this.isOpen=!1,this.billing=new y,this.selectedPlan="passq_growth_monthly",this.init()}init(){typeof document>"u"||(document.addEventListener("click",e=>{const a=e.target.closest("#btn-open-subscription-modal, [data-open-subscription-modal]");if(a){e.preventDefault();const o=a.getAttribute("data-plan")||"passq_growth_monthly";this.open(o)}}),document.addEventListener("keydown",e=>{e.key==="Escape"&&this.isOpen&&this.close()}),s.onLanguageChange(async()=>{this.isOpen&&(await s.loadNamespaces(["common"]),this.render())}))}async open(e="passq_growth_monthly"){this.selectedPlan=e,await s.loadNamespaces(["common"]),this.isOpen=!0,this.render(),document.body.classList.add("overflow-hidden")}close(){this.isOpen=!1,this.modalEl&&(this.modalEl.classList.add("opacity-0","pointer-events-none"),setTimeout(()=>{!this.isOpen&&this.modalEl&&(this.modalEl.remove(),this.modalEl=null)},200)),document.body.classList.remove("overflow-hidden")}render(){let e=document.getElementById("passq-subscription-modal-root");e||(e=document.createElement("div"),e.id="passq-subscription-modal-root",document.body.appendChild(e)),this.modalEl=e;const a=this.billing.getTrialStatus(),o=s.t("common:billing.badge","Google Play & PayPal • 7 Días de Prueba Gratis"),n=s.t("common:billing.title","Suscripciones Oficiales PassQ"),l=s.t("common:billing.subtitle","Suscríbete con total seguridad a través de Google Play Store (móvil) o directamente con PayPal / Tarjeta de débito o crédito (web y computadoras)."),i=s.t("common:billing.trial_badge","7D"),d=s.t("common:billing.trial_status_active","Estado de Prueba Actual: Día {day} de 7 ({remaining} días restantes)"),m=s.t("common:billing.trial_status_ready","Prueba de 7 Días Disponible (Inicia con tu 1er pasaporte)"),u=s.t("common:billing.trial_status_ended","Prueba Concluida");let c=m,h=s.t("common:billing.trial_desc_ready","Tu prueba gratuita de 7 días se activará automáticamente cuando crees tu primer pasaporte digital.");a.isTrialStarted&&(a.isTrialActive?(c=d.replace("{day}",a.day).replace("{remaining}",a.daysRemaining),h=s.t("common:billing.trial_desc","Tienes acceso completo e ilimitado a todas las herramientas Pro. Al suscribirte ahora, no se te cobrará nada hasta terminar tus 7 días de prueba.")):(c=u,h=s.t("common:billing.trial_desc_ended","Tu periodo de prueba ha finalizado. Suscríbete para continuar emitiendo y gestionando pasaportes digitales.")));const v=s.t("common:billing.founder_rate","Tarifa Fundador"),f=s.t("common:billing.features.days_free","✓ 7 Días Gratis"),w=s.t("common:billing.features.play_billing","✓ Google Play / PayPal"),g=s.t("common:billing.guarantees_title","Garantías y Seguridad Oficial:"),b=s.t("common:billing.guarantee_1","Google Play Store: Facturación oficial respaldada por Google con comprobante fiscal."),_=s.t("common:billing.guarantee_2_prefix","PayPal Seguro: Protección al comprador. Paga con saldo PayPal o tarjeta de débito/crédito."),$=s.t("common:billing.guarantee_3","Cero almacenamiento de datos bancarios o tarjetas en servidores de PassQ."),M=s.t("common:billing.btn_subscribe_play_main","Google Play Store"),C=s.t("common:billing.btn_subscribe_play_sub","(App Android • 7 Días Gratis)"),S=s.t("common:billing.btn_subscribe_paypal_main","Pagar con PayPal / Tarjeta"),L=s.t("common:billing.btn_subscribe_paypal_sub","(Web & PC • Activación Inmediata)"),A=s.t("common:buttons.close","Cerrar");e.className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-6 bg-slate-950/80 backdrop-blur-md transition-opacity duration-200",e.innerHTML=`
      ...
      <div class="relative w-full max-w-2xl max-h-[88vh] max-h-[88dvh] overflow-y-auto overscroll-contain rounded-3xl bg-white/95 dark:bg-[#0c1322]/95 border border-slate-200 dark:border-white/15 shadow-2xl p-5 sm:p-8 backdrop-blur-2xl z-10 animate-in fade-in zoom-in-95 duration-200 flex flex-col gap-5">
        ...
        <div class="flex items-center justify-between">
          <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-500/15 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 text-xs font-black tracking-wide">
            <span class="w-4 h-4 flex items-center">${r.shieldCheck?r.shieldCheck("w-4 h-4"):"🛡️"}</span>
            <span>${o}</span>
          </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
          ${k.map(p=>{const j=this.selectedPlan===p.id,P=s.t(`common:billing.plans.${p.nameKey}.name`,p.nameKey.toUpperCase()),E=s.t(`common:billing.plans.${p.nameKey}.limit`,"PassQ Pro");return`
              <div data-plan-select="${p.id}" class="p-4 rounded-2xl border-2 transition-all cursor-pointer flex flex-col justify-between relative select-none ${j?"border-emerald-500 bg-emerald-500/10 shadow-lg shadow-emerald-500/10":"border-slate-200/80 dark:border-white/10 bg-slate-50 dark:bg-white/5 hover:border-slate-300 dark:hover:border-white/20"}">
                ${p.recommended?`
        ...
        <div class="p-3.5 rounded-2xl bg-slate-100/80 dark:bg-white/5 border border-slate-200/80 dark:border-white/10 text-xs text-slate-600 dark:text-slate-300 space-y-1.5">
          <div class="flex items-center gap-2 font-bold text-slate-900 dark:text-white">
            <span class="w-4 h-4 text-emerald-500">${r.shieldCheck?r.shieldCheck("w-4 h-4"):"✓"}</span>
            <span>${g}</span>
          </div>
        ...
        <div class="flex flex-col sm:flex-row items-stretch gap-3 pt-1">
          ...
          <button id="btn-confirm-paypal-subscribe" type="button" class="flex-1 min-h-[58px] py-3.5 px-4 rounded-2xl bg-[#0070ba] hover:bg-[#005ea6] text-white shadow-xl shadow-blue-600/25 transition active:scale-[0.98] flex items-center justify-center gap-3 cursor-pointer">
            <span class="w-6 h-6 flex items-center shrink-0 text-white">${r.paypal?r.paypal("w-6 h-6"):"🅿"}</span>
            <div class="flex flex-col items-center sm:items-start text-center sm:text-left leading-tight">
              <span class="font-black text-sm sm:text-base tracking-tight">${S}</span>
          ...
          <button id="btn-confirm-play-subscribe" type="button" class="flex-1 min-h-[58px] py-3.5 px-4 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white shadow-xl shadow-emerald-600/25 transition active:scale-[0.98] flex items-center justify-center gap-3 cursor-pointer">
            <span class="w-6 h-6 flex items-center shrink-0">${r.googlePlay?r.googlePlay("w-6 h-6"):"▶"}</span>
            <div class="flex flex-col items-center sm:items-start text-center sm:text-left leading-tight">
    ...
    `}setupListeners(){const e=this.container.querySelector("#custom-lang-btn"),a=this.container.querySelector("#custom-lang-menu");!e||!a||(e.addEventListener("click",o=>{o.stopPropagation(),this.toggleMenu()}),a.addEventListener("click",o=>{const n=o.target.closest(".lang-option-btn");if(!n)return;o.stopPropagation();const l=n.getAttribute("data-lang-code");l&&(s.setLanguage(l),this.closeMenu())}),document.addEventListener("click",o=>{this.isOpen&&!this.container.contains(o.target)&&this.closeMenu()}),document.addEventListener("keydown",o=>{o.key==="Escape"&&this.isOpen&&this.closeMenu()}))}toggleMenu(){this.isOpen?this.closeMenu():this.openMenu()}openMenu(){const e=this.container?.querySelector("#custom-lang-menu"),a=this.container?.querySelector("#custom-lang-chevron"),o=this.container?.querySelector("#custom-lang-btn");if(!e)return;this.isOpen=!0,this.container&&(this.container.style.zIndex="9999");const n=this.container.closest("header")||this.container.closest(".glass-card");n&&(n.style.zIndex="999"),e.classList.remove("hidden"),o&&o.setAttribute("aria-expanded","true"),requestAnimationFrame(()=>{const l=e.getBoundingClientRect();l.left<8?(e.classList.remove("right-0","origin-top-right"),e.classList.add("left-0","origin-top-left")):l.right>window.innerWidth-8&&(e.classList.remove("left-0","origin-top-left"),e.classList.add("right-0","origin-top-right")),e.classList.remove("opacity-0","scale-95"),e.classList.add("opacity-100","scale-100"),a&&a.classList.add("rotate-180")})}closeMenu(){const e=this.container?.querySelector("#custom-lang-menu"),a=this.container?.querySelector("#custom-lang-chevron"),o=this.container?.querySelector("#custom-lang-btn");e&&(this.isOpen=!1,o&&o.setAttribute("aria-expanded","false"),e.classList.remove("opacity-100","scale-100"),e.classList.add("opacity-0","scale-95"),a&&a.classList.remove("rotate-180"),setTimeout(()=>{if(!this.isOpen){e.classList.add("hidden"),this.container&&(this.container.style.zIndex="");const n=this.container?.closest("header")||this.container?.closest(".glass-card");n&&(n.style.zIndex="")}},150))}updateActiveState(e){const a=x.find(i=>i.code===e)||x[0],o=this.container?.querySelector("#custom-lang-current-flag"),n=this.container?.querySelector("#custom-lang-current-label");o&&r[`flag_${a.code}`]&&(o.innerHTML=r[`flag_${a.code}`]("w-4 h-3 rounded-xs shadow-xs shrink-0")),n&&(n.textContent=a.short);const l=this.container?.querySelector("#custom-lang-menu > div");l&&(l.innerHTML=x.map(i=>{const d=i.code===e,m=r[`flag_${i.code}`]?r[`flag_${i.code}`]("w-4 h-3 rounded-xs shadow-xs shrink-0"):"";return`
      ...
      <div class="relative w-full max-w-xl max-h-[88vh] max-h-[88dvh] overflow-y-auto overscroll-contain rounded-3xl bg-white/95 dark:bg-[#0c1322]/95 border border-slate-200 dark:border-white/15 shadow-2xl p-5 sm:p-8 backdrop-blur-2xl z-10 animate-in fade-in zoom-in-95 duration-200 flex flex-col gap-5">
        ...
        <div class="flex items-center justify-between">
          <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-semibold tracking-wide">
            <span class="w-4 h-4 flex items-center">${r.chatBubble?r.chatBubble("w-4 h-4"):""}</span>
            <span>${o}</span>
          </div>
        ...
        <div class="space-y-1.5">
          <h2 class="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2.5">
            <span class="p-2 rounded-xl bg-emerald-500/10 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400">
              ${r.envelope?r.envelope("w-5 h-5"):""}
            </span>
            <span>${a}</span>
          </h2>
          <p class="text-sm font-semibold text-emerald-600 dark:text-emerald-400">${n}</p>
        </div>
        ...
        <div class="p-4 sm:p-5 rounded-2xl bg-emerald-500/5 dark:bg-emerald-500/10 border border-emerald-500/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <button id="btn-copy-email-action" type="button" class="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs sm:text-sm font-semibold shadow-xs transition active:scale-95 cursor-pointer">
            <span id="copy-icon-slot" class="w-4 h-4 flex items-center">${r.copy?r.copy("w-4 h-4"):""}</span>
            <span id="copy-text-slot">${u}</span>
          </button>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-2">
          <a href="${f}" id="btn-send-mail-action" class="flex-1 inline-flex items-center justify-center gap-2.5 px-6 py-3 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-sm sm:text-base shadow-lg shadow-emerald-600/25 transition active:scale-[0.98] cursor-pointer">
            <span class="w-5 h-5 flex items-center">${r.envelope?r.envelope("w-5 h-5"):""}</span>
            <span>${m}</span>
          </a>
        ...
        <div class="flex items-center justify-center gap-2 text-center text-xs text-slate-500 dark:text-slate-400 pt-1">
          <span class="w-3.5 h-3.5 text-emerald-500 shrink-0">${r.shieldCheck?r.shieldCheck("w-3.5 h-3.5"):"✓"}</span>
          <span>${c}</span>
        </div>

      </div>
    `,e.querySelector("#contact-modal-backdrop")?.addEventListener("click",()=>this.close()),e.querySelector("#btn-close-contact-modal")?.addEventListener("click",()=>this.close()),e.querySelector("#btn-cancel-contact-modal")?.addEventListener("click",()=>this.close());const w=e.querySelector("#btn-copy-email-action");w&&w.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(d);const g=e.querySelector("#copy-icon-slot"),b=e.querySelector("#copy-text-slot");g&&b&&(g.innerHTML=r.checkCircle?r.checkCircle("w-4 h-4 text-emerald-500"):"✓",b.textContent=s.t("contact_modal.email_copied","¡Copiado!"),setTimeout(()=>{g.innerHTML=r.copy?r.copy("w-4 h-4"):"",b.textContent=u},2500))}catch(g){console.warn("Clipboard write error:",g)}})}}new z;const G=new I;export{r as I,D as L,N as a,s as i,O as p,V as r,G as t};
